...
class TuyaAccessory extends EventEmitter {
    ...
    _connect() {
        ...
        if (this._socket) {
            ...
            if (this._socket._connTimeout) clearTimeout(this._socket._connTimeout);
            if (this._socket._errorReconnect) clearTimeout(this._socket._errorReconnect);
        ...
        this._socket.on('data', msg => {
            ...
            while (this._cachedBuffer.length >= 16) {
                ...
                if (startingIndex !== 0) {
                    ...
                    if (this._cachedBuffer.length < 16) break;
                }

                const size = this._cachedBuffer.readUInt32BE(12);
                const packetLen = 16 + size;
                if (this._cachedBuffer.length < packetLen) {
                    break;
                }

                const packet = this._cachedBuffer.subarray(0, packetLen);
                this._cachedBuffer = this._cachedBuffer.subarray(packetLen);

                if (packet.readUInt32BE(packetLen - 4) !== 0x0000aa55) {
                    this.log.error('Malformed packet suffix from ' + this.context.name + ':', packet.toString('hex'));
                    continue;
                }

                this._msgQueue.push({msg: packet});
            }
        });
    ...
    _msgHandler_3_4(task, callback) {
        ...
        if (expectedCrc !== computedCrc) {
            this.log.error(`HMAC mismatch for ${this.context.name}: expected ${expectedCrc}, was ${computedCrc}. Raw: ${task.msg.toString('hex')}`);
            return callback();
        }
        ...
        if (cmd === 4) { // CommandType.RENAME_GW
            ...
            if (expLocalHmac !== calcLocalHmac) {
                this.log.error(`HMAC mismatch (keys) for ${this.context.name}: expected ${expLocalHmac}, was ${calcLocalHmac}. Raw: ${parsedPayload.toString('hex')}`);
                return callback();
            }
            const payload = {
