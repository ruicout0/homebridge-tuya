...
class DehumidifierAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        infoService.getCharacteristic(Characteristic.Manufacturer).updateValue(this.device.context.manufacturer || 'Tuya');
        infoService.getCharacteristic(Characteristic.Model).updateValue(this.device.context.model || 'Dehumidifier');
        ...
        if (this.device.context.noSpeed) {
            // Remove Fan service from cached accessories when noSpeed is enabled
            const existingFan = this.accessory.getService(Service.Fan);
            if (existingFan) {
                this.log('Removing cached Fan service from', this.device.context.name);
            ...
            fanService.getCharacteristic(Characteristic.On)
                .updateValue(this._getActive(dps[this.getDp('Active')]) === Characteristic.Active.ACTIVE)
                .on('get', (callback) => {
                ...
                .on('set', (value, callback) => {
                    this.setState(this.getDp('Active'), value, callback);
                });
            characteristicSpeed = fanService.getCharacteristic(Characteristic.RotationSpeed)
                ...
                .updateValue(this._getRotationSpeed(dps[this.getDp('FanSpeed')]))
        ...
        this.device.on('change', (changes, state) => {
            if (changes.hasOwnProperty(this.getDp('Active'))) {
                ...
                if (characteristicActive.value !== newActive) {
                    ...
                    if (!this.device.context.noSpeed) {
                        ...
                        if (fanService) {
                            fanService.getCharacteristic(Characteristic.On).updateValue(!!changes[this.getDp('Active')]);
                        }
                    }

                    if (!changes.hasOwnProperty(this.getDp('FanSpeed')) && characteristicSpeed) {
                        characteristicSpeed.updateValue(this._getRotationSpeed(state[this.getDp('FanSpeed')]));
                    }
                }
            }
            ...
            if (changes.hasOwnProperty(this.getDp('FanSpeed'))) {
                const newSpeed = this._getRotationSpeed(state[this.getDp('FanSpeed')]);
