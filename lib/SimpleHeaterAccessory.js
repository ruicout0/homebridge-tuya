    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpDesiredTemperature)) {
                const newThreshold = this._getDividedState(changes[this.dpDesiredTemperature], this.thresholdTemperatureDivisor);
                if (characteristicHeatingThresholdTemperature.value !== newThreshold)
                    characteristicHeatingThresholdTemperature.updateValue(newThreshold);
            }
